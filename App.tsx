...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const handlePopState = (e: PopStateEvent) => {
      ...
      // Manejar paneles (cart, wishlist)
      if (state?.panel) {
        if (state.panel === 'cart') {
          setIsCartOpen(true);
        } else if (state.panel === 'wishlist') {
          setIsWishlistOpen(true);
        }
      } else {
        // Cerrar todos los paneles si no se especifica
        setIsCartOpen(false);
  ...
  const navigateToSection = (sectionName: string) => {
    if (sectionName === 'Ver Todo' || sectionName === 'Catálogo') {
      ...
      setIsWishlistOpen(true);
      window.history.pushState(
        { view: currentView, panel: 'wishlist' },
        '',
        '#wishlist'
      );
                        ...
                        {cargandoProductos ? (
                          ...
                          Array.from({ length: 6 }).map((_, index) => (
                            <div key={index} className="animate-pulse">
                              <div className="bg-gray-200 h-80 rounded-sm mb-4"></div>
